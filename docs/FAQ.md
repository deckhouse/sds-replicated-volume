...
description: LINSTOR Troubleshooting. When should I use LVM, and when LVMThin? Performance and reliability notes, comparison to Ceph. How to add existing LVM or LVMThin pool. How to configure Prometheus to storing data. Controller's work-flow questions.
---
...
## When should I use LVM, and when LVMThin?

Compare the two storage options:

- LVM is a simpler option that provides high performance close to that of the underlying drive itself. However, LVM does not support snapshots.
- LVMThin allows you to create snapshots and use overprovisioning, which makes it more flexible, but with lower overall performance compared to LVM.

**Caution.** Use overprovisioning in LVMThin carefully and monitor free space in the pool. The cluster monitoring system notifies you when free space drops to the 20%, 10%, 5%, and 1% thresholds. If the pool completely runs out of free space, module degradation and a risk of data loss are possible.
...
There are three [`replication`](./cr.html#replicatedstorageclass-v1alpha1-spec-replication) modes:

| Mode | Description | When to use |
|---|---|---|
| `None` | A single data replica. Equivalent to a regular PV resource, with no fault tolerance or availability. | Suitable for test environments or clustered applications (for example, a multi-node cluster of RabbitMQ/MongoDB/MySQL). |
| `Availability` | Two data replicas + one diskless replica (contains no data; when used, it accesses any available "full" replica over the network). Provides a certain level of availability if one replica fails but does not guarantee data consistency (integrity). | A compromise mode for non-critical data and applications that need some high availability (for example, when nodes periodically go into maintenance) but do not have strict reliability or data integrity requirements. In case of network connectivity issues (one of the quorum replicas is diskless, and accessing data through it happens over the network), desynchronization and, optionally, data loss are possible. |
| `ConsistencyAndAvailability` | Three data replicas, full fault tolerance with guaranteed data preservation in case of failures. | The most reliable replication mode. Recommended for mission-critical applications, vital data, and deploying virtual machines in a DVP environment. |

By default, the `ConsistencyAndAvailability` mode is used when creating a [ReplicatedStorageClass](./cr.html#replicatedstorageclass). Specify it as follows:
...
**Caution.** This information reflects the total available space in the cluster. If you need to create volumes in two replicas, divide these values by two to understand how many such volumes can fit in the cluster.
...
**Caution.** This information reflects the total available space in the cluster. When creating volumes with two replicas, these two replicas must fit entirely across two nodes of your cluster.

## How do I set the default StorageClass?

Add the corresponding StorageClass name to `spec.settings.defaultClusterStorageClass` of the `ModuleConfig/global` resource.
...
To expand an existing [ReplicatedStoragePool](./cr.html#replicatedstoragepool) with a new LVM Volume Group, follow these steps:

1. Create a new [LVMVolumeGroup](/modules/sds-node-configurator/cr.html#lvmvolumegroup) with [sds-node-configurator](/modules/sds-node-configurator/usage.html#creating-an-lvmvolumegroup-resource).
...
**Caution.** For greater stability of the module, do not reboot multiple nodes simultaneously.
...
1. Check that there are no problematic resources in DRBD or resources in `SyncTarget`. If there are any, wait for synchronization or take measures to restore normal operation.
...
1. Reboot the node and wait for **all** DRBD resources to synchronize. Then uncordon the node. If you need to reboot another node, repeat the procedure.
...
## How to free space in a Storage Pool by moving resources to another node
...
1. Fix all faulty resources in the cluster. Run the following command to filter them:
...
**Caution.** After the script finishes its job, the node will still be in the Kubernetes cluster albeit in *SchedulingDisabled* status. In LINSTOR, the *AutoplaceTarget=false* property will be set for this node, preventing its scheduler from creating resources on this node.
...
- `--ignore-advise` — Proceeds with the operations despite warnings from the `linstor advise resource` command. Use if the script was interrupted and the number of replicas for some resources does not match the value specified in the [ReplicatedStorageClass](./cr.html#replicatedstorageclass).
- `--exclude-resources-from-check` — Excludes from checks the resources listed using the `|` (vertical bar) as a separator.


## How to diagnose volume problems?
...
### Why does linstor-node fail to start because the DRBD module cannot be loaded?
...
1. If some of those pods are stuck in the `Init` state, check the DRBD version as well as the bashible logs on the node:
...
- You may have the distribution-shipped DRBDv8 module loaded, while DRBDv9 is required. To check the loaded module version, use the following command:

  ```shell
  cat /proc/drbd
  ```

  If the file is missing, the module is not loaded and this is not the cause.
...
- You may have Secure Boot enabled.
  ...
  Currently, running the DRBD module with Secure Boot enabled is not supported.

### Why can't the Pod start due to the FailedMount error?

#### The Pod is stuck at the ContainerCreating phase

If the Pod is stuck at the `ContainerCreating` phase, and if the errors like those shown below are displayed when the `d8 k describe pod` command is invoked, then it means that the device is mounted on one of the nodes.
...
Yes, this is the expected behavior. Currently, the `sds-replicated-volume` module does not process operations when deleting the [ReplicatedStoragePool](./cr.html#replicatedstoragepool) resource.

## I am unable to update the fields in the ReplicatedStorageClass resource spec. Is this the expected behavior?  

Yes, this is the expected behavior. All [`spec`](./cr.html#replicatedstorageclass-v1alpha1-spec) fields of the [ReplicatedStorageClass](./cr.html#replicatedstorageclass) resource are immutable.
...
Operations that require manual intervention are partially or fully automated in the `sds-replicated-volume` module. Therefore, the module restricts the list of allowed LINSTOR commands. For example, Tie-Breaker creation is automated — LINSTOR itself sometimes does not create them for resources with two replicas. You can view the list of allowed commands by running:
...
for MOBJECT in $MOBJECTS; do
  ...
  d8 k get rsmb "$MOBJECT" -o jsonpath="{.spec.data}" | base64 --decode >> "$COMBINED"
done
```
...
Restore the desired entity by applying the YAML file:
...
1. Check [`dataNodes.nodeSelector`](./configuration.html#parameters-datanodes-nodeselector) in the module settings:

   ```shell
   d8 k get mc sds-replicated-volume -o=jsonpath={.spec.settings.dataNodes.nodeSelector}
   ```

1. Check the selectors that `sds-replicated-volume-controller` uses:
...
1. Confirm that the `d8-sds-replicated-volume-controller-config` secret contains the selectors specified in the module settings, as well as the `kubernetes.io/os: linux` selector.

1. Make sure that the target node has all the labels specified in the `d8-sds-replicated-volume-controller-config` secret:

   ```shell
   d8 k get node worker-0 --show-labels
   ```

1. If there are no labels, add them to the NodeGroup or to the node via templates.

1. If there are labels, check whether the target node has the `storage.deckhouse.io/sds-replicated-volume-node=` label. If there is no label, check whether `sds-replicated-volume-controller` is running and examine its logs:
...
Information about the reasons for the failed operation should appear in the `status.reason` field of the [ReplicatedStoragePool](./cr.html#replicatedstoragepool) and [ReplicatedStorageClass](./cr.html#replicatedstorageclass) resources.
If the information provided is not enough to identify the problem, you can refer to the `sds-replicated-volume-controller` logs.

## How do I migrate from the built-in Deckhouse Kubernetes Platform [linstor](/modules/linstor/) module to sds-replicated-volume?

During the migration, the `LINSTOR` control plane and its CSI will be unavailable. This will make it impossible to create/expand/delete PVs and create/delete pods that use its PVs for the duration of the migration.

**Caution.** The migration will not affect user data, because the move is to a new namespace and new components will be added that will later take over volume management functionality.
...
```
   ...
   **Caution.** It is important to make all resources healthy before migrating.
...
1. Create a ModuleConfig resource for `sds-replicated-volume`.
   ...
   **Caution.** Failing to specify the [`settings.dataNodes.nodeSelector`](./configuration.html#parameters-datanodes-nodeselector) parameter in the `sds-replicated-volume` module settings would result in the value for this parameter to be derived from the `linstor` module when installing the `sds-replicated-volume` module. If this parameter is not defined there as well, it will remain empty and all the nodes in the cluster will be treated as storage nodes.
...
1. Override the `linstor` command alias and check the resources:
...
Note that StorageClasses in this module are managed via the [ReplicatedStorageClass](./cr.html#replicatedstorageclass) resource. StorageClasses should not be created manually.

When migrating from the linstor module, delete old StorageClasses and create new ones via the [ReplicatedStorageClass](./cr.html#replicatedstorageclass) resource (refer to the table below).

Note that in the old StorageClasses, you should pick up the option from the parameter section of the StorageClass itself, while for the new StorageClass, you should specify the corresponding option in [ReplicatedStorageClass](./cr.html#replicatedstorageclass).
...
- [`reclaimPolicy`](./cr.html#replicatedstorageclass-v1alpha1-spec-reclaimpolicy) (`Delete`, `Retain`) — corresponds to the reclaimPolicy parameter of the old StorageClass.
- [`zones`](./cr.html#replicatedstorageclass-v1alpha1-spec-zones) — list of zones to be used for hosting resources (the actual names of the zones in the cloud). Please note that remote access to a volume with data is only possible within a single zone!
- [`volumeAccess`](./cr.html#replicatedstorageclass-v1alpha1-spec-volumeaccess) can be `Local` (access is strictly within the node), `EventuallyLocal` (the data replica will be synchronized on the node with the running pod a while after the start), `PreferablyLocal` (remote access to the volume with data is allowed, `volumeBindingMode: WaitForFirstConsumer`), `Any` (remote access to the volume with data is allowed, `volumeBindingMode: Immediate`).
- If you need to use `volumeBindingMode: Immediate`, set the [`volumeAccess`](./cr.html#replicatedstorageclass-v1alpha1-spec-volumeaccess) parameter of the [ReplicatedStorageClass](./cr.html#replicatedstorageclass) to `Any`.

You can read more about working with [ReplicatedStorageClass](./cr.html#replicatedstorageclass) resources [in the documentation](./usage.html).
...
During the migration, the module control plane and its CSI will be unavailable. This will make it impossible to create, expand, or delete PVs and create or delete pods that use DRBD PVs for the duration of the migration.

**Caution.** The migration will not affect user data, because the move is to a new namespace and new components will be added that will later take over the module's volume management functionality.
...
1. Make sure there are no faulty DRBD resources in the cluster. The command below should return an empty list:
   ...
   **Caution.** It is important to make all `DRBD` resources healthy before migrating.
...
1. Create a ModuleConfig resource for `sds-replicated-volume`.
   ...
   **Caution.** Failing to specify the [`settings.dataNodes.nodeSelector`](./configuration.html#parameters-datanodes-nodeselector) parameter in the `sds-replicated-volume` module settings would result in the value for this parameter to be derived from the `sds-drbd` module when installing the `sds-replicated-volume` module. If this parameter is not defined there as well, it will remain empty and all the nodes in the cluster will be treated as storage nodes.
...
- Redundant RAID dramatically increases the overhead in terms of space utilization.
  Here is an example: Suppose, a [ReplicatedStorageClass](./cr.html#replicatedstorageclass) is used with [`replication`](./cr.html#replicatedstorageclass-v1alpha1-spec-replication) set to `ConsistencyAndAvailability`. With this setting, DRBD will store data in three replicas (one replica per three different hosts). If RAID1 is used on these hosts, a total of 6 GB of disk space will be required to store 1 GB of data. Redundant RAID is worth using for easier server maintenance when the storage costs are irrelevant. RAID1 in this case will allow you to change disks on servers without having to move data replicas from the "problem" disk.
...
1. Check the certificate expiration dates in the module secrets:
...
1. Wait until Deckhouse becomes `Ready` and the queue is empty:
