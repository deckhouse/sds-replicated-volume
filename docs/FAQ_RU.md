...
3. Этот ресурс можно указать в параметрах [DRBDStoragePool](ссылка на ресурс) в поле `spec.lvmVolumeGroups[].name` (для LVMThin-пула необходимо дополнительно указать имя в `spec.lvmVolumeGroups[].thinPoolName`).
