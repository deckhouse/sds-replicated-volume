...
В модуле `sds-drbd` мы ограничили список команд, которые разрешено выполнять в LINSTOR, потому что все операции по ручному вмешательству мы планируем автоматизировать. Часть из них мы уже автоматизировали, например, создание Tie-Breaker, когда LINSTOR их не создает для ресурсов с 2мя репликами. Список разрешенных команд можно посмотреть так:
...
## Как выполнить миграцию со встроенного модуля linstor на модуль sds-drbd?
...
> **Важно!** Пользовательские данные миграцией затронуты не будут. По сути произойдет переезд в новый namespace и добавление новых компонентов, которые в будущем возьмут на себя весь функционал `LINSTOR` по управлению томами.

### Порядок действий для миграции

- Удостовериться, что в кластере нет "плохих ресурсов" `LINSTOR`. Эта команда должна выводить пустой список:
...
- Выключить модуль `linstor`:

```shell
kubectl patch moduleconfig linstor --type=merge -p '{"spec": {"enabled": false}}'
```

- Дождаться, когда namespace `d8-linstor` будет удален.

```shell
kubectl get namespace d8-linstor
```

- Создать ресурс `ModuleConfig` для `sds-node-configurator`.
...
- Дождаться, когда модуль `sds-node-configurator` перейдет в состояние `Ready`.
...
- Дождаться, когда модуль `sds-drbd` перейдет в состояние `Ready`.

```shell
kubectl get moduleconfig sds-drbd
```

- Проверить настройки модуля `sds-drbd`

```shell
kubectl get moduleconfig sds-drbd -oyaml
```

- Дождаться, пока все поды в namespace `d8-sds-drbd` и `d8-sds-node-configurator` перейдут в состояние `Ready` или `Completed`.
...
- Изменить алиас к команде `linstor` и проверить ресурсы `LINSTOR`:
