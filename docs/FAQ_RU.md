...
Зоны (zones) — это пользовательская абстракция, определяющая, на каких узлах будут создаваться volumes. Принадлежность узла к той или иной зоне отображается в `Labes` узла по ключу `topology.kubernetes.io/zone`. 
