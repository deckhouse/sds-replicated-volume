...
- LVM — это более простой вариант, который обеспечивает высокую производительность, близкую к производительности самого накопителя. Однако LVM не поддерживает использование snapshot'ов.
- LVMThin — позволяет создавать snapshot'ы и применять overprovisioning, что делает его более гибким, но с понижением общей производительности по сравнению с LVM.
...
1. Переведите узел в состояние drain:
...
1. Перезагрузите узел, и дождитесь синхронизации **всех** DRBD-ресурсов. Выполните uncordon узла. При необходимости перезагрузки еще одного узла — повторитt алгоритм.
...
1. Переместите ресурсы на другой узел (только 1-2 ресурса одновременно):
...
Для запуска скрипта `evict.sh` в неинтерактивном режиме необходимо добавить флаг `--non-interactive` при его вызове, а также имя узла, с которого необходимо вытеснить ресурсы. В этом режиме скрипт выполнит все действия без запроса подтверждения от пользователя. Пример вызова:
