...
## Как вручную перевыпустить сертификаты?

Чтобы инициировать процесс ручного продления сертификатов, создайте `ConfigMap` с именем `manualcertrenewal-trigger`:
...
Статус операции можно определить по значению свойства `data.step` триггера:
...
Сертификаты выдаются сроком на один год и помечаются как устаревающие за 30 дней до истечения срока действия.

Чтобы повторить операцию, удалите метку `storage.deckhouse.io/sds-replicated-volume-manualcertrenewal-completed` у триггера.
