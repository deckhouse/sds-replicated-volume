...
├─────────────────────────────────────────────────────────────────┤
│ 1. CREATE(RV, sharedSecret == "")                               │
│ 2. CREATE/UPDATE(RVR, ConfigurationAdjusted=False,              │
...
│ 2. Проверить: sharedSecret установлен?                          │
└─────────────────────────────────────────────────────────────────┘
                            │
                ┌───────────┴───────────┐
                │                       │
                ▼                       ▼
    ┌───────────────────┐   ┌──────────────────────────┐
    │ sharedSecret == ""│   │ sharedSecret установлен  │
    └───────────────────┘   └──────────────────────────┘
                │                       │
                ▼                       ▼
    ┌───────────────────────┐  ┌──────────────────────────────┐
    │ generateSharedSecret()│  │ handleUnsupportedAlgorithm() │
    └───────────────────────┘  └──────────────────────────────┘
                │                       │
                │                       │
    ┌───────────▼───────────┐   ┌───────▼───────────────────────┐
    │ 1. Генерирует UUID    │   │ 1. Получает все RVR           │
    │ 2. Выбирает sha256    │   │ 2. Фильтрует по volumeName    │
    │    (первый алгоритм)  │   │ 3. Ищет UnsupportedAlgorithm  │
    │ 3. Устанавливает      │   └───────────────────────────────┘
    │    SharedSecret=UUID  │              │
    │    SharedSecretAlg=   │              │
    │    sha256             │              │
    │ 4. Устанавливает      │   ┌──────────▼──────────────┐
    │    condition=True     │   │ Есть ошибки?            │
    └───────────────────────┘   └─────────────────────────┘
                                        │
                        ┌───────────────┴───────────────┐
                        │                               │
            ...
            │ → return (idempotent) │      │ → Переключить alg  │
            └───────────────────────┘      └────────────────────┘
                                                    │
                                    ┌───────────────┴───────────────┐
                                    │                               │
                                    ▼                               ▼
                        ┌──────────────────────┐      ┌──────────────────────┐
                        │ Есть следующий alg?  │      │ Алгоритмы исчерпаны  │
                        └──────────────────────┘      └──────────────────────┘
                                │                               │
                                ▼                               ▼
                ┌───────────────────────────┐      ┌──────────────────────────┐
                │ Да: sha256 → sha1         │      │ setAlgorithmSelection    │
                │ 1. Генерирует новый UUID  │      │ Failed()                 │
                │ 2. Переключает на sha1    │      │ 1. Устанавливает         │
                │ 3. Устанавливает          │      │    condition=False       │
                │    condition=True         │      │ 2. reason=               │
                └───────────────────────────┘      │    UnableToSelect...     │
                                                   │ 3. message с узлами      │
                                                   └──────────────────────────┘
```
...
**Триггеры:**
- `CREATE(RV)` без `sharedSecret` → запускает генерацию
- `CREATE/UPDATE(RVR)` с ошибкой `UnsupportedAlgorithm` → запускает переключение алгоритма

**Основной поток:**
1. **Генерация при создании:** Если `sharedSecret` не установлен, генерируется UUID и выбирается первый алгоритм (`sha256`)
2. **Обработка ошибок:** Если `sharedSecret` установлен, проверяются RVR на наличие ошибок `UnsupportedAlgorithm`
3. **Переключение алгоритма:** При наличии ошибок переключается на следующий алгоритм и генерируется новый `sharedSecret`
4. **Исчерпание алгоритмов:** Если все алгоритмы исчерпаны, устанавливается условие `False` с информацией об ошибке

**Важно: Логика проверки алгоритма:**
- **Что проверяется:** Проверяем наличие ошибки `UnsupportedAlgorithm` в RVR для **текущего алгоритма**, установленного в `rv.Status.Config.SharedSecretAlg`
- **Что НЕ проверяется:** Мы НЕ проверяем, работает ли другой алгоритм (например, если установлен `sha1`, мы НЕ проверяем, работает ли `sha256`)
- **Как работает:** 
  - Если установлен `sha256` → проверяем, есть ли ошибка `UnsupportedAlgorithm` для `sha256` на узлах
  - Если установлен `sha1` → проверяем, есть ли ошибка `UnsupportedAlgorithm` для `sha1` на узлах
  - Ошибка `UnsupportedAlgorithm` устанавливается другим контроллером/агентом, когда они пытаются применить конфигурацию с текущим алгоритмом и не могут (например, DRBD на узле не поддерживает этот алгоритм)

**Особенности:**
- Идемпотентность: проверки на каждом этапе
- Безопасность параллелизма: использование `PatchStatusWithConflictRetry`
- Наблюдаемость: условия в status для диагностики
...
3. **Тестирование:**
   ...
   - Покрытие: 5 тестов, включая happy path, edge cases, идемпотентность
