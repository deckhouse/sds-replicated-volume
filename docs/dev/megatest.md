...
# Пачка го-рутин
## volume-checker(rv)
Собирает статистику по переходам состояния rv ориентируясь на кондишены.
  ...
  - при переключении состояния - писать в лог с Reason и Message если Condition status меняется. Записть в структуру rvName, кол-во переходов для каждого из кондишенов, в начале кондишены должны быть в true. Написать  в лог кондишены rvr == false.
...
Эмулирует работу csi, публикуя rv на разных нодах. 
  - в цикле:
    ...
    - в завивимости от количества нод в PublesdON:
      ...
      - 1 : 
        - wantedNodeName не находится в PublishOn - тогда цикл эмуляции миграции (добавляем новую, убирем из PublishOn старую, затем удаляем новую) (0 нод на выходе)
        - wantedNodeName уже находится в PublishOn - тогда только unpublish цикл (убрать одну ноду) (0 нод на выходе)
      - 2: - кейс когда контролеер упал и поднялся
        - wantedNodeName находится или не находится в PublishOn  - делаем Unblish цикл, удаляем случайную (убрать одну ноду) (1 на выходе).
  ...
  - когда получает сигнал окончания
    - делает действие анпаблиш
      ...
      - дожидается успеха 
    - выходит
## volume-resizer(rv, period_min, period_max, step_min, step_max) - ОТЛОЖЕН!
Меняет размеры rv.
...
## volume-main (rv, sc, lifetime_period)
  ...
  - выполняет действие создать rv
    ...
    - запускает:
      ...
      - volume-resizer(rv, 50, 50, 4kb, 64kb) - ОТЛОЖЕНО! - контролер рейсаза может увеличить rv больше чем запроешено, если это требуется на более низком уровне, поэтому проверка должна уто учитывать. Но нужно уточнить порог срабатывания  sds-node-configurator - он может не увеличивать на малые значения. 
...
Удаляет поды control-plane по label_selector
  - в цикле:
    ...
    - выбирает поды с заданным label_selector, перемешиывает список (на статус не смотрит)
