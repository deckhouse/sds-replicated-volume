...
## volume-checker(rv)
  ...
  - при переключении состояния - гадить в лог с Reason и Message если Contion status меняется, а так же  время переключения, считать количество раз и показать в конце(записать в структуру для сбора в стату для вернего уровня). Без инфы от rvr, по крайней мере пока. В стте: имя rv, False-> True, True->False коунтеры для каждого из кондишенов, или рассмотреть возмонсть более детально с Reason.
  - когда получает сигнал окончания — выходит
## volume-publisher (rv, period_min, period_max, publishedRVNum)
  - в цикле:
    - ждет рандом
    - в завивимости от publishedRVNum:
      - 0 - только unpublish
      - 1 - Publish на 1 ноду - TBD
      - 2 -  TBD
     ниже все TBD,  надо дописать на учитывая распределение каким то образом
---
    - случайным образом выбирает ноду с label sds-replicated-volume.
    ...
    - делает действие анпаблиш **выбранной ноды**(выше), если publichOn содержит эту ноду. 
      ...
      - гадит в лог о любых действиях или бездействиях(когда ноды 2)
...
## volume-replica-destroyer (rv, period_min, period_max)
  - в цикле пока не выйдем, с случайным интервалом из (period_min+max)
    ...
    - выполняет действие удаления:
      ...
      - гадит в лог , который уже структурирован, действие
  - когда получает сигнал окончания
    - выходит
## volume-replica-creator (rv, period_min, period_max)
  - в цикле пока не выйдем, с случайным интервалом из (period_min+max)
    ...
    - выполняет действие создания rvr c выбранным типом.
      - создает rvr
      - НЕ дожидается успеха
      - гадит в лог, который уже структурирован, тип и действие
  - когда получает сигнал окончания
    - выходит
## volume-main (rv, sc, lifetime_period)
  ...
  - выполняет действие создать rv
    - создает rv
    - запускает:
      - volume-publisher(rv, 30, 60,0)
      - volume-publisher(rv, 100, 200,1)
      - volume-publisher(rv, 100, 200,2)
