...
> **Caution!** If the cluster has only a single node, it is recommended to use either `sds-local-volume` or `local-path-provisioner` instead of `sds-replicated-volume`.
