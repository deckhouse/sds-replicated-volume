...
### Настройка хранилища на узлах, где планируется хранить реплики данных

Для этого нужно на этих узлах через пользовательские ресурсы `Kubernetes` создать `LVM Volume Groups`. В быстром старте будем создавать обычное `Thick` хранилище. Подробнее про пользовательские ресурсы и примеры их использования можно прочитать в [примерах использования](./usage.html).
...
- Проверить, что в LINSTOR создался Storage Pool `data` на узлах `worker-0` и `worker-1`:
