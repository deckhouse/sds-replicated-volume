...
После включения модуля `SDS-DRBD` в конфигурации Deckhouse ваш кластер будет автоматически настроен на использование бэкенда `LINSTOR`. Останется только создать [пулы хранения и StorageClass'ы](./usage.html#конфигурация-хранилища-linstor).
