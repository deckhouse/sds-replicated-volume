...
if [ -n "$drbd_version" ]; then
  ...
  (set -x; sed 's/$version[[:space:]]*:=[[:space:]]*"[0-9]\+\.[0-9]\+\.[0-9]\+"/$version := "'$drbd_version'"/' -i images/drbd/werf.inc.yaml)
  (set -x; sed 's/drbd[0-9]\+/drbd'$drbd_version_undotted'/' -i templates/nodegroupconfiguration-drbd-install-*.yaml)
fi
