  ...
  rules:
    - alert: D8LinstorControllerGrowingErrorReports
      ...
      annotations:
        ...
        description: |
          ...
          1. Check the Pod logs: `kubectl -n d8-sds-replicated-volume logs deploy/linstor-controller`
          2. Check the LINSTOR error reports: `kubectl exec -n d8-sds-replicated-volume deploy/linstor-controller -- linstor err list | grep 'C|linstor-controller'`
    ...
    - alert: D8LinstorControllerTargetDown
      ...
      annotations:
        ...
        description: |
          The recommended course of action:
          1. Check the Pod status: `kubectl -n d8-sds-replicated-volume get pod -l app=linstor-controller`
          2. Or check the Pod logs: `kubectl -n d8-sds-replicated-volume logs deploy/linstor-controller`
    ...
    - alert: D8LinstorControllerPodIsNotReady
      expr: min by (pod) (kube_pod_status_ready{condition="true", namespace="d8-sds-replicated-volume", pod=~"linstor-controller-.*"}) != 1
      labels:
        severity_level: "6"
        tier: cluster
      for: 10m
      annotations:
        ...
        description: |
          The recommended course of action:
          1. Retrieve details of the Deployment: `kubectl -n d8-sds-replicated-volume describe deploy linstor-controller`
          2. View the status of the Pod and try to figure out why it is not running: `kubectl -n d8-sds-replicated-volume describe pod -l app=linstor-controller`

    - alert: D8LinstorControllerPodIsNotRunning
      expr: absent(kube_pod_status_phase{namespace="d8-sds-replicated-volume",phase="Running",pod=~"linstor-controller-.*"})
      labels:
        severity_level: "6"
        tier: cluster
      for: 2m
    ...
    - alert: D8LinstorControllerIncorrectDRBDPortRange
      expr: count(kube_configmap_labels{namespace="d8-sds-replicated-volume",label_storage_deckhouse_io_incorrect_port_range="true"}) > 0
      labels:
        severity_level: "3"
        tier: cluster
      for: 2m
      annotations:
        ...
        plk_create_group_if_not_exists__d8_linstor_controller_health: "D8LinstorControllerHealth,tier=~tier,prometheus=deckhouse,kubernetes=~kubernetes"
        plk_grouped_by__d8_linstor_controller_health: "D8LinstorControllerHealth,tier=~tier,prometheus=deckhouse,kubernetes=~kubernetes"
        summary: The linstor-controller DRBD port range is incorrect
        description: |
          The recommended course of action:
