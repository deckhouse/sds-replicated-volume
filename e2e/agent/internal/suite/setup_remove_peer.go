...
// SetupRemovePeer patches the DRBDResource to remove all peers, waits for
// configured, and asserts no peers remain in status.
func SetupRemovePeer(
	e envtesting.E,
	cl client.WithWatch,
	...
	if len(drbdr.Status.Peers) != 0 {
		e.Fatalf("DRBDResource %q still has %d peers after removal", drbdr.Name, len(drbdr.Status.Peers))
	}
