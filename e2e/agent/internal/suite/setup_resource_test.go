...
func TestSetupResource_WithoutPredicate(t *testing.T) {
	...
	result := suite.SetupResource(f.e, f.wc, cm, nil)
...
func TestSetupResource_WithPredicate(t *testing.T) {
	...
	var result *corev1.ConfigMap
		...
		result = suite.SetupResource(f.e.ScopeWithTimeout(testTimeout), f.wc, cm, func(cm *corev1.ConfigMap) bool {
...
func TestSetupResource_CleanupRemovesFinalizers(t *testing.T) {
	f := newFixture(t)
	suite.SetupResource(f.e, f.wc, newConfigMap(nil), nil)

	cm := mustGetConfigMap(t, f.client)
	cm.Finalizers = []string{"blocking-finalizer"}
...
func TestSetupResourcePatch_WithoutPredicate(t *testing.T) {
	...
	result := suite.SetupResourcePatch(f.e, f.wc, testKey, func(cm *corev1.ConfigMap) {
		cm.Data["key"] = "patched"
	}, nil)
...
func TestSetupResourcePatch_WithPredicate(t *testing.T) {
	...
	var result *corev1.ConfigMap
		...
		result = suite.SetupResourcePatch(f.e.ScopeWithTimeout(testTimeout), f.wc, testKey,
...
func TestSetupResourcePatch_RevertOnCleanup(t *testing.T) {
	...
	suite.SetupResourcePatch(f.e, f.wc, testKey, func(cm *corev1.ConfigMap) {
		cm.Data["a"] = "changed"
		cm.Data["c"] = "new"
	}, nil)
