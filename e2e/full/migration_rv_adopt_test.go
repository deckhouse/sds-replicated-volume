...
var _ = Describe("Migration: RV adopt/v1 formation with preexisting DRBD", Label(fw.LabelUpgrade, fw.LabelSlow), func() {
	DescribeTable("adopts preexisting replicas and completes formation",
		...
		// Entry("FTT=0 GMDR=1 secondary (2 replicas)", require.MinNodes(2), byte(0), byte(1), 2, false),
		// Entry("FTT=0 GMDR=1 primary (2 replicas)", require.MinNodes(2), byte(0), byte(1), 2, true),
		// Entry("FTT=1 GMDR=0 secondary (2 replicas)", require.MinNodes(2), byte(1), byte(0), 2, false),
		// Entry("FTT=1 GMDR=0 primary (2 replicas)", require.MinNodes(2), byte(1), byte(0), 2, true),
		// Entry("FTT=1 GMDR=1 secondary (3 replicas)", require.MinNodes(3), byte(1), byte(1), 3, false),
		// Entry("FTT=1 GMDR=1 primary (3 replicas)", require.MinNodes(3), byte(1), byte(1), 3, true),
	)
})
