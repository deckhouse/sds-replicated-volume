...
srvBackupLabelKey = 'sds-replicated-volume.deckhouse.io/sds-replicated-volume-db-backup-weekly'

def create_backup(backup_type, labels={}):
    temp_path = tempfile.mkdtemp()
    backup_archive = tarfile.open(f'{temp_path}/sds-replicated-volume_weekly{backup_type}.tar', "w")
    ...
    with open(f'{temp_path}/sds-replicated-volume_weekly{backup_type}.tar', 'rb') as backup_archive:
        encoded_string = base64.b64encode(backup_archive.read())

    step = 262144 # 256KB
    for chunk_pos in range(0, len(encoded_string) // step + 1, 1):
        objName = f'sds-replicated-volume-{backup_type}-backup-{chunk_pos}'
...
if __name__ == "__main__":
    ...
    success = create_backup(backup_type=date_time, labels={srvBackupLabelKey: f'{date_time}', "completed": "true"})
    if not success:
