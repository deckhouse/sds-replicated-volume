...
if __name__ == "__main__":
    ...
    body = kubernetes.client.V1Secret(
        ...
        metadata=kubernetes.client.V1ObjectMeta(name=f'linstor-{date_time}-backup-completed',
                                                namespace=namespace), labels={linstor_backup_label: f'{date_time}'})
    kubernetes.client.CoreV1Api().create_namespaced_secret(namespace=namespace, body=body)

    namespace_secrets = kubernetes.client.CoreV1Api().list_namespaced_secret(namespace=namespace, label_selector={"sds-replicated-volume.deckhouse.io/linstor-db-backup": f'{date_time}'})
