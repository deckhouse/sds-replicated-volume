...
var errStatusNoSuchResource = errors.New("no such resource")
...
var StatusKnownErrors = []KnownError{
	{ExitCode: 10, OutputSubstring: "No such resource", JoinErr: errStatusNoSuchResource},
}
...
func ExecuteStatus(ctx context.Context, resourceName string) (StatusResult, error) {
	...
	cmd := ExecCommandContext(ctx, DRBDSetupCommand, StatusArgs(resourceName)...)
	jsonBytes, err := executeCommand(cmd, StatusKnownErrors)
	if err != nil {
		if resourceName != "all" && errors.Is(err, errStatusNoSuchResource) {
