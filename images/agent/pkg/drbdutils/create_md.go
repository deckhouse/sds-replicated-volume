...
// executeDumpMD runs drbdmeta dump-md and reports whether valid metadata was
// found, returning the raw output so callers can parse fields out of it.
//
// Failures are classified through DumpMDKnownErrors rather than a hand-rolled
// exit-code check, so both drbd-utils conventions (exit 1 on <= 9.31.0, 255 on
// >= 9.32.0) stay handled in one place.
func executeDumpMD(ctx context.Context, minor uint, backingDev string) (output []byte, hasMD bool, err error) {
	...
	outStr := string(out)
	for _, ke := range DumpMDKnownErrors {
		if ke.ExitCode != exitCode || !strings.Contains(outStr, ke.OutputSubstring) {
			continue
		}
		if errors.Is(ke.JoinErr, ErrNoValidMetadata) {
			return nil, false, nil
		}
		if errors.Is(ke.JoinErr, ErrUncleanMetadata) {
			// Metadata exists but the activity log needs applying. The dump
			// output is still valid for extracting on-disk fields.
			return out, true, nil
		}
	}

	return nil, false, fmt.Errorf("running command %s: %w", cmd, withOutput(cmdErr, out))
}
