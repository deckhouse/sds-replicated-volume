...
var ResourceOptionsKnownErrors = []KnownError{
	{ExitCode: 10, OutputSubstring: "(158)", JoinErr: ErrResourceOptionsResourceNotFound},
}
...
func ExecuteResourceOptions(ctx context.Context, resource string, opts ResourceOptions) error {
	cmd := ExecCommandContext(ctx, DRBDSetupCommand, ResourceOptionsArgs(resource, opts)...)
	_, err := executeCommand(cmd, ResourceOptionsKnownErrors)
	return err
}
