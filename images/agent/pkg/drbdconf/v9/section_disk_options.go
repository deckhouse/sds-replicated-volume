...
type DiskOptions struct {
	...
	ALUpdates *ALUpdatesValue

	// Use disk barriers to make sure that requests are written to disk in the right
	// order. Barriers ensure that all requests submitted before a barrier make it
	// to the disk before any requests submitted after the barrier. This is
	// implemented using 'tagged command queuing' on SCSI devices and 'native
	...
	// that requests are stable on disk when they finish. Before DRBD 8.0.9, this
	// was the only method implemented. This option is enabled by default. Do not
	// disable in production environments.
	//
	// From these three methods, drbd will use the first that is enabled and
	// supported by the backing storage device. If all three of these options are
	// turned off, DRBD will submit write requests without bothering about
	...
	// finish an I/O request within the defined disk-timeout, DRBD treats this as a
	// failure. The lower-level device is detached, and the device's disk state
	// advances to Diskless. If DRBD is connected to one or more peers, the failed
	// request is passed on to one of them.
	//
	...
	// blocked/hung local backing devices which do no longer complete requests at
	// all, not even do error completions. In this situation, usually a hard-reset
	// and failover is the only way out.
	//
	// By "aborting", basically faking a local error-completion, we allow for a more
	// graceful swichover by cleanly migrating services. Still the affected node has
	// to be rebooted "soon".
	//
	...
	// 64K-striping, 128K-striping, 256K-striping, 512K-striping and 1M-striping.
	//
	// This option is available since DRBD 8.4.1.
	ReadBalancing *ReadBalancingPolicy

	// Define that a device should only resynchronize after the specified other
	// device. By default, no order between devices is defined, and all devices will
	...
	DiscardZeroesIfAligned *DiscardZeroesIfAlignedValue
	...
	DisableWriteSame *DisableWriteSameValue
}

type ALUpdatesValue string

const (
	ALUpdatesValueYes ALUpdatesValue = "yes"
	ALUpdatesValueNo  ALUpdatesValue = "no"
)
...
type DiscardZeroesIfAlignedValue string

const (
	DiscardZeroesIfAlignedValueYes DiscardZeroesIfAlignedValue = "yes"
	DiscardZeroesIfAlignedValueNo  DiscardZeroesIfAlignedValue = "no"
)

type DisableWriteSameValue string

const (
	DisableWriteSameValueYes DisableWriteSameValue = "yes"
	DisableWriteSameValueNo  DisableWriteSameValue = "no"
)
