...
type On struct {
	...
	//
	// The address families ipv4, ipv6, ssocks (Dolphin Interconnect Solutions'
	// "super sockets"), sdp (Infiniband Sockets Direct Protocol), and sci are
	// supported (sci is an alias for ssocks). If no address family is
	// specified, ipv4 is assumed. For all address families except ipv6, the
	// address is specified in IPV4 address notation (for example, 1.2.3.4). For
	// ipv6, the address is enclosed in brackets and uses IPv6 address notation
	// (for example, [fd01:2345:6789:abcd::1]). The port is always specified as
	// a decimal number from 1 to 65535.
	//
	// On each host, the port numbers must be unique for each address; ports
	// cannot be shared.
	...
	//
	// Node identifiers can only be reasssigned in a cluster when the cluster is
	// down. It is essential that the node identifiers in the configuration and
	// in the device metadata are changed consistently on all hosts. To change
	// the metadata, dump the current state with drbdmeta dump-md, adjust the
	// bitmap slot assignment, and update the metadata with drbdmeta restore-md.
	//
