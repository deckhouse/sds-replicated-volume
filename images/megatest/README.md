...
--rv-size                            # размер ReplicatedVolume или верхняя граница случайного диапазона (Kubernetes quantity, минимум 100Mi; default: 100Mi)
--rv-size-min                        # нижняя граница случайного размера ReplicatedVolume (Kubernetes quantity, опционально)
--rv-size-step                       # шаг случайного размера ReplicatedVolume (Kubernetes quantity; default: 100Mi)
...
Перед стартом создания ресурсов megatest печатает сводку эффективных параметров, включая стратегию выбора размера ReplicatedVolume, включенные раннеры и chaos-настройки. После вывода сводки есть задержка 500ms, чтобы оператор успел увидеть конфигурацию до начала создания объектов.

Размер ReplicatedVolume задается как Kubernetes quantity (`100Mi`, `500Mi`, `1Gi`). Если указан только `--rv-size`, все тома создаются одного размера. Если указан `--rv-size-min`, размер каждого нового тома выбирается случайно в диапазоне `--rv-size-min..--rv-size` с шагом `--rv-size-step`; крайние значения диапазона тоже могут выпадать.
...
## Горутины
### volume-checker(rv)
Собирает статистику по переходам состояния rv по полям **EffectiveLayout** (контроллер вычисляет их на каждом цикле реконсиляции).
  - следит (через shared informer), что с rv все ок:
    - **FTT** (FailuresToTolerate) >= 0 — кворум есть; &lt; 0 — кворум потерян; nil — нет данных
    - **GMDR** (GuaranteedMinimumDataRedundancy) >= 0 — IO/данные доступны; &lt; 0 — IO заблокирован; nil — нет данных
  - готовность rv к использованию (для ожидания после создания) определяется так: **DatameshRevision &gt; 0** и **нет активного transition с типом Formation**
  - при переключении состояния — пишет в лог (transition, значение FTT/GMDR, EffectiveLayout.Message). Записывает в структуру rvName, кол-во переходов по FTT и по GMDR; в начале оба считаются healthy. При unhealthy логирует RVR с condition == false.
  - четное кол-во переходов — rv восстановился; нечетное — закончил в плохом состоянии. В идеале счетчики по нулям.
  ...
  - в цикле:
    ...
    - **Обычный цикл** (добавим одну и уберем одну):
      - делает действие паблиш: **создаёт RVA** для выбранной ноды (не затрагивая другие RVA).
        - дожидается успеха: у RVA для выбранной ноды `Ready=True` (агрегат `Attached=True` и `ReplicaReady=True`).
      - ждет рандом
      - делает действие анпаблиш **выбранной ноды**: удаляет соответствующую RVA (если она существует)
        - дожидается успеха: RVA для выбранной ноды удалена.
    ...
    - **Attach цикл** (только добавить 1 ноду):
      - делает действие паблиш: создаёт RVA для выбранной ноды
        - дожидается успеха: у RVA для выбранной ноды `Ready=True`.
        - пишет в лог
    - **Цикл эмуляции миграции** (создаём новую RVA, удаляем старую RVA, затем удаляем новую)
      - делает действие паблиш: создаёт RVA для выбранной новой ноды
        - дожидается успеха: у RVA для выбранной новой ноды `Ready=True`.
      - действие анпаблиш **старой ноды**: удаляет RVA старой ноды
        - дожидается успеха: RVA для старой ноды удалена.
        - пишет в лог о любых действиях или бездействиях (когда ноды 2)
      - ждет рандом
      - действие анпаблиш **выбранной новой ноды**: удаляет RVA выбранной новой ноды
        - дожидается успеха: RVA для выбранной новой ноды удалена.
        - пишет в лог о любых действиях или бездействиях (когда ноды 2)
