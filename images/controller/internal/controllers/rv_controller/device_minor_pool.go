...
func (c *DeviceMinorPoolInitializer) doInitialize(ctx context.Context) (*idpool.IDPool, error) {
	...
	sort.SliceStable(rvs, func(i, j int) bool {
		ai := meta.IsStatusConditionTrue(rvs[i].Status.Conditions, v1alpha1.ConditionTypeDeviceMinorAssigned)
		aj := meta.IsStatusConditionTrue(rvs[j].Status.Conditions, v1alpha1.ConditionTypeDeviceMinorAssigned)
		if ai == aj {
			return false
		}
		return ai && !aj
