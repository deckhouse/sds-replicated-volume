...
### 3.1 GMDR (Guaranteed Minimum Data Redundancy)
...
So GMDR = 1, not 2, even though 3 disk copies may exist when all nodes are healthy.

### 3.2 FTT (Failures To Tolerate)
...
### 3.3 ADR (Actual Data Redundancy)
...
Unlike GMDR (which is the worst-case guarantee determined by `qmr`), ADR reflects
the actual number of data copies at this moment. During normal operation **ADR ≥ GMDR** always.

Example: a 2D+1TB layout (GMDR=0, qmr=1) has ADR=1 when both D replicas are
UpToDate, but GMDR=0 because the system is *permitted* to continue IO with 1 copy (via
tiebreaker after a temporary failure).

### 3.4 Key Formulas (GMDR / FTT → DRBD settings)

```
qmr = GMDR + 1
D   = FTT + GMDR + 1    (minimum diskful nodes)
q   = floor(D / 2) + 1          (minimum safe quorum value)
TB  = 1  if D is even AND FTT = D / 2, else 0
```

**Why D = FTT + GMDR + 1**: After losing FTT diskful nodes (worst case), the
remaining `D − FTT` nodes must still satisfy `qmr`: `D − FTT ≥ qmr = GMDR + 1`.
...
(= FTT), only `D/2` remain, which is `q − 1` — one vote short. The tiebreaker
bridges this gap (if `qmr` is still met). Without TB, the maximum FTT for even D is
only `D/2 − 1`.

### 3.5 Valid Combinations (|GMDR − FTT| ≤ 1, both ∈ {0, 1, 2})
...
### 5.2 GMDR=0, FTT=1 → `2D + 1TB (q=2, qmr=1)`
...
down), writes go to only 1 disk. Permanent loss of that disk = data lost (GMDR=0).
...
always have all data (`qmr=2`, Protocol C), 1 permanent disk loss is safe (GMDR=1).

---

### 5.4 GMDR=1, FTT=1 → `3D (q=2, qmr=2)`
...
Permanent loss of both → data lost. GMDR=1.
...
### 5.5 GMDR=1, FTT=2 → `4D + 1TB (q=3, qmr=2)`
...
disk loss → 1 copy remains, data safe. GMDR=1.
...
to 3 disks. Permanent loss of 2 of those 3 → 1 copy remains. GMDR=2.
...
sees 2 missing nodes), exceeding FTT=1.

---

### 5.7 GMDR=2, FTT=2 → `5D (q=3, qmr=3)`
...
of 2 of those 3 → 1 copy remains. GMDR=2.
...
| GMDR | ADR | FTT | Layout       | q | qmr | D | TB | Total | Min copies during IO | Write latency (Protocol C) |
|---------|----------|---------|--------------|---|-----|---|----|-------|---------------------|---------------------------|
...
|----------|-------------|-------|-----|
| Development / ephemeral / externally replicated | GMDR=0, FTT=0 (1D) | 1 | Minimal resources; OK if data is replicated by other means |
| Production, availability-first | GMDR=0, FTT=1 (2D+1TB) | 3 | Stays available through 1 failure; 2 copies when healthy |
| Production, data-safety-first | GMDR=1, FTT=0 (2D) | 2 | Always writes to 2 disks; stops on any failure |
| Production, balanced | GMDR=1, FTT=1 (3D) | 3 | 2+ copies guaranteed, tolerates 1 failure |
| Production, high availability | GMDR=1, FTT=2 (4D+1TB) | 5 | 2+ copies guaranteed, tolerates 2 failures |
| Mission-critical, data safety | GMDR=2, FTT=1 (4D) | 4 | 3+ copies guaranteed, tolerates 1 failure |
| Mission-critical, full | GMDR=2, FTT=2 (5D) | 5 | 3+ copies guaranteed, tolerates 2 failures |
...
- **GMDR < FTT** (e.g. 0, 1): System prioritizes availability — stays running even
...
1. **Never reduce GMDR/FTT guarantees** below the target level
...
quorum (arr=false, non-voter), this does not affect any GMDR/FTT guarantees.
...
### 8.1 Replacing a replica in 1D (GMDR=0, FTT=0)
...
| # | Layout | q | qmr | GMDR | ADR | FTT | Action | Wait condition |
|---|--------|---|-----|---------|----------|---------|--------|----------------|
...
the **full resync duration** (hours for large volumes). FTT remains 0 (tolerance of 0
...
**Note**: ADR is 0 in the table (sD copies don't count as D), but data physically
exists on 2 disks during sD sync step.

### 8.2 Replacing a replica in 2D + 1TB (GMDR=0, FTT=1)

**Goal**: Replace D replica #0 with new D replica #2. Maintain GMDR≥0 and FTT≥1 throughout.
...
**FTT=1 verification** — single-failure scenarios at step 1 (worst case during transition, 3 voters):
...
**ADR is 1 throughout** and rises to 2 at step 2 (three UpToDate).
...
### 8.3 Replacing a replica in 2D (GMDR=1, FTT=0)

**Goal**: Replace D replica #0 with new D replica #2. Maintain GMDR≥1 throughout.
...
replicas must stay UpToDate for IO to continue. This is already the baseline (FTT=0),
...
**GMDR=1 maintained**: `qmr=2` never changes → 1 permanent loss is always survivable.
...
layout's FTT=0 (where any D failure stops IO). The D∅ is a "free" addition that
...
### 8.4 Replacing a replica in 3D (GMDR=1, FTT=1)

**Goal**: Replace D replica #0 with new D replica #3. Maintain GMDR≥1 and FTT≥1.
...
time, 4 voters with q=3. FTT=1 is maintained throughout (see verification below).

#### 8.4b With sD

sD pre-syncs data invisibly, reducing the D∅ resync step from full to delta.
...
**FTT=1 verification** (worst case: 4 voters, q=3, with D∅):
...
**FTT=1 throughout** in both variants.

### 8.5 Replacing a replica in 4D + 1TB (GMDR=1, FTT=2)

**Goal**: Replace D replica #0 with new D replica #4. Maintain GMDR≥1 and FTT≥2.
...
**FTT=2 verification at step 1** (worst case, 5 voters + D∅ + TB):
...
### 8.6 Replacing a replica in 4D (GMDR=2, FTT=1)

**Goal**: Replace D replica #0 with new D replica #4. Maintain GMDR≥2 and FTT≥1.
...
**FTT=1 verification at step 1** (5 voters, q=3, qmr=3):
...
### 8.7 Replacing a replica in 5D (GMDR=2, FTT=2)

**Goal**: Replace D replica #0 with new D replica #5. Maintain GMDR≥2 and FTT≥2.
...
**FTT=2 verification** (worst case: 6 voters, q=4, with D∅):
...
**FTT=2 throughout** in both variants.
...
**Goal**: Replace TB replica #0 with new TB replica #1. Maintain GMDR≥0 and FTT≥1.
...
**FTT=1 verification at step 1** (2D + 2TB):
...
TBs for tiebreaker). The FTT=1 **guarantee is not affected** — a single failure
...
**Goal**: Replace TB replica #0 with new TB replica #1. Maintain GMDR≥1 and FTT≥2.
...
**FTT=2 verification at step 1** (4D + 2TB):
...
FTT=2 guarantee is not affected. Operational risk is slightly higher during the 2TB
...
Any change of GMDR or FTT is performed as a sequence of **single-parameter steps**,
where each step changes exactly one parameter by ±1. Like §8, all transitions must:

1. **Never reduce GMDR/FTT** below the minimum of source and target values
2. **Never interrupt IO** (maintain quorum throughout)
3. **Never create split-brain risk** (maintain safe quorum settings)
...
When both GMDR and FTT change, apply the following priority at each step:

- **Upgrade**: prefer increasing GMDR first. If the edge doesn't exist
  (|GMDR−FTT| > 1 constraint), increase FTT instead.
- **Downgrade**: prefer decreasing FTT first. If the edge doesn't exist, decrease
  GMDR instead.

This rule produces paths through **pure-D layouts** (no TB management), which are simpler:

```
Upgrade (0,0)→(2,2):   1D →GMDR→ 2D →FTT→ 3D →GMDR→ 4D →FTT→ 5D
Downgrade (2,2)→(0,0): 5D →FTT→ 4D →GMDR→ 3D →FTT→ 2D →GMDR→ 1D
```

The alternative (FTT-first on upgrade) would go through 2D+TB and 4D+TB — adding and
removing TB twice. GMDR-first avoids this entirely.

**Why GMDR first on upgrade**: data safety before availability. If the process is
interrupted mid-way, the system has better data protection even if availability isn't
fully upgraded yet. **Why FTT first on downgrade**: reduce availability (the "cheaper"
...
|---|------|-----------|---|----|---|-----|---------|------------|
| 1 | 1D ↔ 2D | GMDR ±1 | 1↔2 | 0 | 1↔2 | 1↔2 | vestibule + qmr | §10.1 |
| 2 | 1D ↔ 2D+TB | FTT ±1 | 1↔2 | 0↔1 | 1↔2 | 1 | D + TB ordering | §10.2 |
| 3 | 2D ↔ 3D | FTT ±1 | 2↔3 | 0 | 2 | 2 | D only | §8 add/remove (§8.3a/§8.4a) |
| 4 | 2D+TB ↔ 3D | GMDR ±1 | 2↔3 | 1↔0 | 2 | 1↔2 | D + TB + qmr | §10.3 |
| 5 | 3D ↔ 4D | GMDR ±1 | 3↔4 | 0 | 2↔3 | 2↔3 | vestibule + qmr | Same pattern as §10.1 (§8.4a + qmr) |
| 6 | 3D ↔ 4D+TB | FTT ±1 | 3↔4 | 0↔1 | 2↔3 | 2 | D + TB ordering | Same pattern as §10.2 (§8.4a + §9) |
| 7 | 4D ↔ 5D | FTT ±1 | 4↔5 | 0 | 3 | 3 | D only | §8 add/remove (§8.6a/§8.7a) |
| 8 | 4D+TB ↔ 5D | GMDR ±1 | 4↔5 | 1↔0 | 3 | 2↔3 | D + TB + qmr | Same pattern as §10.3 (§8.5a + qmr + §9) |
...
the quorum check `utd ≥ qmr` holds (2 ≥ 2 ✓). GMDR rises from 0 to 1 at this step.
...
Step 0 lowers qmr first — GMDR drops from 1 to 0 (= target level). This is safe:
lowering qmr relaxes the quorum check (2 ≥ 1 ✓). Steps 1–3 are the "remove" half of §8.1a.

**Guarantee verification**: min(source, target) = GMDR≥0, FTT≥0. Both hold at every step.
...
Steps 0–2 are the "add" half of §8.1a. Step 3 adds TB — FTT rises from 0 to 1
...
Step 0 removes TB first — FTT drops from 1 to 0 (= target level). Steps 1–3 are the
"remove" half of §8.1a.
...
**Guarantee verification**: min(source, target) = GMDR≥0, FTT≥1. At every step: GMDR≥0 ✓,
FTT≥1 ✓ (3 odd voters with q=2 always resolve partitions; after TB added, even 2 voters +
TB maintain FTT=1). Split-brain safe: q=2 is majority for both 3 and 2 voters.
...
- **Zonal** — all replicas are placed within a single zone. Failure domain is a single
  node. A placement guard enforces that new replicas are added to the primary zone
  (the zone with the most voters). Quorum and replacement logic are otherwise
  unaffected. Procedures from §8–§10 apply directly.
- **TransZonal** — replicas are distributed across zones. The failure domain depends on
  the FTT level:

  for some GMDR/FTT combinations it is an entire zone (the layout survives loss
...
how many zones are available, which determines the **failure domain** — what the FTT
guarantee covers.

Two placement strategies:

| GMDR | FTT | Zone FTT | Layout | Zones required | Failure domain | Available after loss of |
|---------|---------|-------------|--------|---------------|---------------|------------------------|
...
Losing K zones loses exactly K replicas. FTT=N means "survive loss of N zones." This
...
In composite mode, FTT has a **dual meaning**: "available after loss of 1 zone OR
FTT nodes."

### 11.2 Zone-Aware Replica Replacement

The §8 and §9 procedures are zone-agnostic — they describe DRBD-level steps (add D∅,
attach, detach, remove). In Zonal and TransZonal topologies, placement guards add
zone-aware constraints:

**Zone placement rule**: The new replica **should** be placed in the zone with the fewest
replicas of its type. The scheduler uses greedy round-robin (fewest D → preferred zone
for Diskful, fewest total replicas → preferred zone for TieBreaker), which naturally
places the replacement into the zone that lost the replica — because that zone now has
fewer replicas than others. When two zones have equal counts, either is equally safe
(zone-FTT is symmetric), and the scheduler picks based on node scoring.

This is a **scheduler preference** (soft constraint), not a hard same-zone rule.

**Zone-level safety is enforced by datamesh guards on both sides:**

- **Placement guards** block additions that would violate zone constraints:
  - **Zonal**: D and TB additions are blocked if the replica's zone differs from the
    primary zone (zone with the most voters).
  - **TransZonal**: D additions are blocked if placing a voter in the target zone
    would cause any single-zone loss to violate quorum (FTT) or data redundancy
    (GMDR). TB additions are blocked if the target zone already has more than one
    D voter (losing that zone would remove both the tiebreaker and multiple voters).
- **Removal guards** (TransZonal only) block removals that would violate zone-level
  GMDR, FTT, or TB coverage guarantees.

This dual enforcement ensures safety regardless of whether replicas are placed by the
scheduler or manually by the user.

**Zone loss recovery**: If the entire zone is unavailable (all nodes in the zone are down),
the scheduler has no eligible candidates in that zone and will place the replica in the
zone with fewest replicas among the remaining zones. Placement guards ensure the chosen
zone is safe.

**No new procedures are needed**: §8.1–§8.7 and §9.1–§9.2 apply as-is. The DRBD quorum
mechanics, q/qmr settings, and vestibule patterns are unaffected by zone placement.
...
zone-level resilience profile. The procedure must maintain zone-FTT throughout.
...
zone-FTT≥1 throughout.

Start: D(zone A) + D(zone B) + TB(zone C).
End: TB(zone A) + D(zone B) + D(zone C).

| # | Layout | Zone A | Zone B | Zone C | q | qmr | GMDR | FTT | Action | Wait condition |
|---|--------|--------|--------|--------|---|-----|---------|---------|--------|----------------|
...
**Zone-FTT=1 verification at each step**:
...
**Zone-FTT verification**: At steps 2–4 there are 5 D voters (odd, q=3 = safe
...
the 2D zone leaves 2D < qmr=3 → quorum lost. Zone-FTT=0 for the biggest zone. TB
cannot help (tiebreaker requires `up_to_date ≥ qmr`, and 2 < 3). For GMDR=2 in 3
zones, use **5D (q=3, qmr=3)** directly.

The available layouts for 3-zone TransZonal with zone-FTT ≥ 1 form a **linear chain**:

```
2D+1TB (0,1) ——GMDR—— 3D (1,1) ——FTT—— 4D+1TB (1,2) ——GMDR—— 5D (2,2)
```

All transitions follow this chain. The §10 ordering rule ("GMDR first") is naturally
satisfied where possible, and the chain avoids 4D entirely:

| # | Transition | Edge | §10 reference |
|---|-----------|------|--------------|
| 1 | 2D+1TB ↔ 3D | GMDR ±1 | §10.3 |
| 2 | 3D ↔ 4D+1TB | FTT ±1 | §10.2 pattern (§8.4a + §9) |
| 3 | 4D+1TB ↔ 5D | GMDR ±1 | §10.3 pattern (§8.5a + qmr + §9) |

Multi-step transitions follow the chain. Example upgrade 3D → 5D:

| # | Layout | Distribution | Zone FTT | GMDR | FTT | Transition |
|---|--------|-------------|-------------|---------|---------|------------|
...
2. **Degraded but available** (within FTT): lost some replicas, IO continues, ADR
   reduced.
   - Start §8 replacement to restore redundancy?
   - What if another replica fails during replacement (cascading failure)?
   - Priority: replace the replica that restores the most ADR first?

3. **FTT exceeded** (quorum lost, IO suspended): too many replicas lost.
   - **Option A**: lower GMDR/FTT (change q/qmr via §10 downgrade) to match
