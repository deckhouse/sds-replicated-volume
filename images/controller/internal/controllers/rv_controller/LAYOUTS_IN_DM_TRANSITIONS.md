...
The §8 and §9 DM transition sequences apply as-is with additional zone constraints:

**Zone placement rule**: The scheduler uses greedy round-robin (fewest replicas of the
target type → preferred zone), which naturally places the replacement into the zone that
lost the replica. When two zones have equal counts, either is equally safe (zone-FTT is
symmetric). This is a soft preference, not a hard same-zone constraint.

**Placement guards** enforce zone safety for **additions**:

- **Zonal**: D and TB additions are blocked if the replica's zone differs from the
  primary zone (zone with the most voters).
- **TransZonal**: D additions are blocked if adding the voter to the target zone would
  make any single-zone loss violate FTT or GMDR. TB additions are blocked if the target
  zone already has more than one D voter (TB should go to a minority-voter zone).

**Removal guards** enforce zone safety for **removals**:

- ZoneGMDR, ZoneFTT, and ZoneTBRequired block removals that would violate zone-level
  guarantees (see DATAMESH_MEMBERSHIP.md §Preconditions).

Together, placement guards and removal guards ensure zone-level safety throughout
the entire replacement lifecycle — not just by placement preference.

**Zone loss recovery**: If the entire zone is unavailable, the scheduler places the
replica in the zone with fewest replicas among the remaining zones.
