...
// adminLockRVSName is the RVS every test in this file operates on.
const adminLockRVSName = "snap-1"
...
func adminLockMakeRVS() *v1alpha1.ReplicatedVolumeSnapshot {
	const name = adminLockRVSName
...
func TestReconcileAcquireAdminLockContinuesWhenRunning(t *testing.T) {
	...
	cond := findCondition(got.Status.Conditions)
	if cond == nil {
...
func findCondition(conds []metav1.Condition) *metav1.Condition {
	const t = v1alpha1.ReplicatedVolumeSnapshotCondAdminLockedType
	for i := range conds {
		if conds[i].Type == t {
			return &conds[i]
		}
