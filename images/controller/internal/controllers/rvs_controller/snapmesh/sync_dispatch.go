...
func syncDispatcher() dmte.DispatchFunc[provider] {
	return func(cp provider) iter.Seq[dmte.DispatchDecision] {
		return func(yield func(dmte.DispatchDecision) bool) {
			...
			for i := range gctx.allReplicas {
				...
				if rctx.rvrs == nil ||
					rctx.rvrs.Status.Phase != v1alpha1.ReplicatedVolumeReplicaSnapshotPhaseReady ||
					rctx.rvrs.Status.SnapshotHandle == "" {
					continue
				}
