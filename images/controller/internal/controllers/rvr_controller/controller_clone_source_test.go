...
var _ = Describe("cloneSourceRVRPredicates", func() {
	...
	makeRVR := func(state v1alpha1.DiskState) *v1alpha1.ReplicatedVolumeReplica {
		const (
			name = "rv-src-0"
			node = "node-1"
		)
	...
	It("accepts Create and Delete events", func() {
		rvr := makeRVR(v1alpha1.DiskStateUpToDate)
	...
	It("rejects updates without meaningful changes", func() {
		oldRVR := makeRVR(v1alpha1.DiskStateUpToDate)
	...
	It("accepts BackingVolume.State changes", func() {
		oldRVR := makeRVR(v1alpha1.DiskStateInconsistent)
		newRVR := makeRVR(v1alpha1.DiskStateUpToDate)
		Expect(pred.Update(event.UpdateEvent{ObjectOld: oldRVR, ObjectNew: newRVR})).To(BeTrue())
	})
