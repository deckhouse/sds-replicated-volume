...
// MaxBackoffDelay is the maximum delay for the per-item exponential backoff rate limiter.
// It is also used as Cache.SyncPeriod so that the informer resync interval matches the
// maximum retry window — if the queue didn't deliver an item within one backoff cycle,
// resync re-enqueues it.
const MaxBackoffDelay = 1 * time.Minute
...
//   - Per-item exponential backoff: 5ms base, MaxBackoffDelay max.
