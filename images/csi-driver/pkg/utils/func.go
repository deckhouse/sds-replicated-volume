...
func DeleteReplicatedVolumeSnapshot(
	...
	log.Trace(fmt.Sprintf("[DeleteReplicatedVolumeSnapshot][traceID:%s][snapshotID:%s] Removing finalizer %s if exists", traceID, name, srv.CSIControllerFinalizer))
	removed, err := removervsdeletepropagationIfExist(ctx, kc, log, rvs, srv.CSIControllerFinalizer)
	if err != nil {
		return fmt.Errorf("remove finalizers from ReplicatedVolumeSnapshot %s: %w", name, err)
	}
	if removed {
		log.Trace(fmt.Sprintf("[DeleteReplicatedVolumeSnapshot][traceID:%s][snapshotID:%s] finalizer %s removed from ReplicatedVolumeSnapshot %s", traceID, name, srv.CSIControllerFinalizer, name))
	} else {
		log.Warning(fmt.Sprintf("[DeleteReplicatedVolumeSnapshot][traceID:%s][snapshotID:%s] finalizer %s not found in ReplicatedVolumeSnapshot %s", traceID, name, srv.CSIControllerFinalizer, name))
	}
...
func removervsdeletepropagationIfExist(ctx context.Context, kc client.Client, log *logger.Logger, rvs *srv.ReplicatedVolumeSnapshot, finalizer string) (bool, error) {
	...
	return false, fmt.Errorf("giving up removing finalizer %s from ReplicatedVolumeSnapshot %s after %d conflicting attempts", finalizer, rvs.Name, KubernetesAPIRequestLimit)
}
