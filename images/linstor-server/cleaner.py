    ...
    def get_resource_list(self, retries=5):
        ...
        for i in range(retries):
            try:
                ...
                logger.debug(f"try retrieve list resource from Linstor {i} time ({e})")
                continue
            else:
                break
        else:
            logger.error("couldn't request resource list from linstor")
            result = None
...
def get_res_files() -> List[str]:
    files = Path(BASE_RES_PATH).glob(RES_FILES_MASK)
    return list(files)
...
def main(interval: int, debug: bool):
    ...
    def run_periodically():
        logger = logging.getLogger()
        print("run_periodically")
        logger.debug("start new iteration")
        process_res_files(log_level)
        scheduler.enter(interval, 1, run_periodically)
